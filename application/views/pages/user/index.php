                        ...
                        <tbody>
                            ...
                            foreach ($content as $row) : $no++; ?>
                                <tr>
                                    ...
                                    <td>
                                        <p>
                                            <img src="<?= $row->image ? base_url("images/user/$row->image") : base_url("images/user/default.png") ?>" width="50px" alt="">
                                            <?= $row->name; ?>
                                        </p>
                                    </td>
                                    <td><?= $row->email; ?></td>
                                    <td><?= $row->role; ?></td>
                                    <td><?= $row->is_active ? 'Tersedia' : 'Kosong'; ?></td>
                                    <td>
                                        <?= form_open(base_url("user/delete/$row->id"), ["method" => "POST"]); ?>
                                        <?= form_hidden('id', $row->id) ?>
                                        <a href="<?= base_url("user/edit/$row->id") ?>"><button class="btn btn-sm" type="button">
